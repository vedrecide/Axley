...
from discord_slash.utils.manage_components import create_button, create_actionrow, ButtonStyle, wait_for_component


class ButtonPaginator:
    def __init__(self, 
            client: Union[discord.Client, commands.Bot], 
            basic: Optional[List[Union[discord.Emoji, discord.PartialEmoji, str]]] = ['Previous', 'Next'], 
            extra: Optional[List[Union[discord.Emoji, discord.PartialEmoji, str]]] = ['Initial', 'Last']
    ) -> None:
        self.client = client
        self.basic = basic
        self.extra = extra
        ...
        if self.client == discord.Client:
            print('<discord.Client> detected. It is highly recommended to use <discord.ext.commands.Bot>.')
        ...
        if self.extra:
            ...
            if isinstance(self.extra[1], discord.Emoji):
                self.left = discord.PartialEmoji(name = self.extra[1].name, animated = self.extra[1].animated, id = self.extra[1].id)
        ...
        while True:
            try:
                inter = await wait_for_component(
                    self.client,
                    components = buttons,
                    check = lambda i: i.component[
                        'label'
                    ] in component_labels and i.origin_message_id == msg.id and i.author.id == context.author.id,
                    timeout = int(timeout)
                )
                await inter.defer(
                    edit_origin = True
