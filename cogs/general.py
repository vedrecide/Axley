...
class General(commands.Cog):
    ...
    async def avatar(self, ctx: commands.Context, member: discord.Member = None):
        ...
        embed.add_field(
            name="Original URL", value="[Click]({})".format(member.avatar_url)
        )
    ...
    async def serverinfo(self, ctx):
        ...
        embed.set_author(
            icon_url=f"{ctx.guild.icon_url}", name=f"Guild Info - {ctx.guild.name}"
        )
        embed.set_thumbnail(url=f"{ctx.guild.icon_url}")
        embed.add_field(name="Owner", value=ctx.guild.owner)
        embed.add_field(name="Guild ID", value=str(ctx.guild.id))
        embed.add_field(
            name="Verification Level", value=str(ctx.guild.verification_level).title()
        )
        ...
        embed.add_field(
            name="Categories", value="`{}` in total".format(len(ctx.guild.categories))
        )
        embed.add_field(
            name="Features",
            value="`" + "` `".join(a for a in ctx.guild.features) + "` ",
            inline=False,
        )
        await ctx.send(embed=embed)
