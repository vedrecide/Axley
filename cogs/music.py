...
class Player(wavelink.Player):
    ...
    def build_embed(self) -> typing.Optional[discord.Embed]:
        ...
        embed = discord.Embed(title=f'Connected to {channel.name}', colour=0xebb145)
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def cog_before_invoke(self, ctx: commands.Context):
        ...
        if player.is_connected:
            if ctx.author not in channel.members:
                await ctx.send(f'{ctx.author.mention}, you must be in `{channel.name}` channel to use voice commands.')
                raise IncorrectChannelError
    ...
    async def filter(self, ctx: commands.Context, *, equalizer: str):
